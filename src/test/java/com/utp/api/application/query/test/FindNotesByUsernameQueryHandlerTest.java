...
public class FindNotesByUsernameQueryHandlerTest {
    ...
    public void testHandle_ReturnsNotes() {
        ...
        NoteDomain note1 = new NoteDomain(1L, 15.5, "Examen parcial", LocalDateTime.now(), username);
        NoteDomain note2 = new NoteDomain(2L, 18.0, "Trabajo final", LocalDateTime.now(), username);
        ...
        assertEquals(note1.getScore(), result.get(0).getScore());
        assertEquals(note1.getDescription(), result.get(0).getDescription());
        
        assertEquals(note2.getId(), result.get(1).getId());
        assertEquals(note2.getScore(), result.get(1).getScore());
        assertEquals(note2.getDescription(), result.get(1).getDescription());
    }
